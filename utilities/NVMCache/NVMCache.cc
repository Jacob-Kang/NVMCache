...
#include "utilities/redis_cache/NVMCache.h"
...
Status NVMCache::Open() {
  ...
  if (opt_.pipeline_writes) {
    ...
    insert_th_ = port::Thread(&NVMCache::InsertMain, this);
  }
...
Status NVMCache::CleanupCacheFolder(const std::string& folder) {
...
PersistentCache::StatsType NVMCache::Stats() {
...
Status NVMCache::Insert(const Slice& key, const char* data,
                              const size_t size) {
  // update stats
...
Status NVMCache::InsertImpl(const Slice& key, const Slice& data) {
  // pre-condition
  assert(key.size());
  assert(data.size());
  assert(cache_file_);
...
Status NVMCache::Lookup(const Slice& key, std::unique_ptr<char[]>* val,
                              size_t* size) {
  StopWatchNano timer(opt_.env, /*auto_start=*/ true);
...
bool NVMCache::Erase(const Slice& key) {
  WriteLock _(&lock_);
...
Status NVMCache::NewCacheFile() {
  lock_.AssertHeld();

  TEST_SYNC_POINT_CALLBACK("NVMCache::NewCacheFile:DeleteDir",
...
bool NVMCache::Reserve(const size_t size) {
  WriteLock _(&lock_);
...
Status NewPersistentCache(Env* const env, const std::string& path,
  ...
  auto pcache = std::make_shared<NVMCache>(opt);
